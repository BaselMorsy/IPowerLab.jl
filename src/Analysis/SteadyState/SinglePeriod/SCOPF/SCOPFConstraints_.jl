...
@with_kw mutable struct SCOPF_SimulationSettings
    ...
    contingency_redispatch = [] # contingency index that should be above 1 since 1 is the base case
...
function single_period_SCOPF_variable_initialization!(model::Model, simulation_settings::SCOPF_SimulationSettings, prerequisites_data::SCOPF_Prerequisites)
    ...
    if simulation_settings.ac_grid_model == :DCOPF
        ...
        JuMP.@variable(model, p_branch[l in prerequisites_data.ac_branch_ids, k in prerequisites_data.k])
        JuMP.@variable(model, p[g in prerequisites_data.ac_gen_set, k in prerequisites_data.k])
    elseif simulation_settings.ac_grid_model == :ACOPF
        ...
        JuMP.@variable(model, δ[i in prerequisites_data.nodes_set, k in prerequisites_data.k])
        JuMP.@variable(model, p_branch[l in prerequisites_data.ac_branch_ids, k in prerequisites_data.k])
        JuMP.@variable(model, q_branch[l in prerequisites_data.ac_branch_ids, k in prerequisites_data.k])
        ...
        error("Invalid grid model setting ($(simulation_settings.ac_grid_model))")
        return -1
    end

    if simulation_settings.dc_grid_model == :Linear
        JuMP.@variable(model, p_branch_dc[i in prerequisites_data.dc_Nodes_set, j in prerequisites_data.dc_Nodes_set, k in prerequisites_data.k; Set([i, j]) in prerequisites_data.dc_Transmission_nodes])
    elseif simulation_settings.dc_grid_model == :NonLinear
        ...
        error("Invalid grid model setting ($(simulation_settings.dc_grid_model))")
        return -1
    end
